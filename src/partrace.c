...
void init_random_particles(Inputs *inputs, double *sizes, double *xs,
                           double *ys, double *zs);
void read_partfile(Inputs *inputs, double *sizes, double *xs, double *ys,
                   double *zs);
...
int main(int argc, char **argv) {
  ...
  if (inputs->modeltype == JUPITER_MODEL) {
    ...
    for (int i = 0; i < NLVL; i++) {
      ...
      models[i] = init_Model(inputs->modeltype, inputs, rank);
    }
    ...
    nlvl = 1;
    models[0] = init_Model(inputs->modeltype, inputs, rank);
    if (rank == 0)
  ...
  if (tf < t0) {
    ...
    BACKWARDS = 1;
    if (inputs->diffusion) {
      printf("!!! ERROR: Integrating backwards with diffusion is not "
             "physically correct and is not recommended !!!\n");
      return 1;
    }
  }
  if (((tf - t0) * dtout) < 0) {
    if (BACKWARDS) {
      printf("Integration is backwards in time but dtout is postive. Setting "
             "dtout = -dtout\n");
    } else {
      printf("Integration is forward in time but dtout is negative. Seting "
             "dtout = -dtout\n");
    }
    dtout = -dtout;
  }
  ...
  if (inputs->residenceTimes) {
    if (nlvl > 1) {
      if (rank == 0)
        printf(
            "Cannot currently track residence times with multilevel model\n");
      MPI_Finalize();
      return 1;
    }
    if (BACKWARDS) {
    ...
    if (nprocs > 1) {
      sprintf(resFilename, "%s/residenceTimes_rank%d.dat", inputs->outputdir,
              rank);
    } else {
    ...
    if (inputs->reset) {
      ...
      double zero = 0;
      // bigSize+1 because we also include the number of particles as the first
      // double
  ...
  if (inputs->crossings) {
    if (nprocs > 1) {
      sprintf(crossFilename, "%s/partCrossings_rank%d.txt", inputs->outputdir,
              rank);
    } else {
  ...
  if (nprocs > 1) {
    sprintf(allpartsFilename, "%s/allparts_rank%d.txt", inputs->outputdir,
            rank);
  } else {
    sprintf(allpartsFilename, "%s/allparts.txt", inputs->outputdir);
  }
  FILE *allpartsf;
  // if the file doesn't exist yet or reset is picked, create it and write the
  // header
  ...
  for (int i = rank; i < np; i += nprocs) {
    ...
    if ((i % inputs->dsave) == 0) {
      sprintf(filename, "%s/particle%d.txt", inputs->outputdir,
              i + inputs->nstart);
    } else {
    ...
    result = integrate(p, t0, tf, dtout, inputs->diffusion, filename,
    ...
    fprintf(allpartsf, "%f\t%f\t%f\t%f\t%f\t%f\t%f\t%d\n", result.tf, xs[i],
            ys[i], zs[i], p->x, p->y, p->z, result.status);
    fclose(allpartsf);
