...
typedef struct MeshField MeshField;
...
void make_cartvels(Model *model, MeshField *gasvphi, MeshField *gasvr,
                   MeshField *gasvtheta);
void init_gradrho(Model *model);
void get_planetVars(Variables *var, Model *model);
double get_soundspeed(Model *model, double r, double mstar, double R0);
Model *init_fargo_Model(Inputs *input, int rank);
Model *init_Jupiter_Model(char *fargodir, char *nout, int rank);
Model *init_RADMC_Model(char *fargodir, char *nout, int rank);

Model *init_Model(int which, Inputs *input, int rank) {
  switch (which) {
  case FARGO_MODEL:
    return init_fargo_Model(input, rank);
  case JUPITER_MODEL:
    return init_Jupiter_Model(input->fargodir, input->nout, rank);
  case RADMC_MODEL:
    return init_RADMC_Model(input->fargodir, input->nout, rank);
  default:
...
Model *init_fargo_Model(Inputs *input,
                        int rank) { // char* fargodir, char* nout,  int rank) {
  char *fargodir = input->fargodir;
  char *nout = input->nout;
  // allocate memory
  ...
  model->domain = init_Domain(fargodir, nx, ny, nz, input->R0);
  ...
  double MASS = input->MSTAR;
  double R0 = input->R0;
  double VOLDENS = MASS / R0 / R0 / R0;
  // cute way to say 1/Omega = sqrt(R0^3/GM)
  double TIME = sqrt(1.0 / VOLDENS / G);
  double SPEED = R0 / TIME;
  double RHOSCALE;
  model->gasdens = init_MeshField_fromFile(rhofile, nx, ny, nz, R0);
  MeshField *gasvphi = init_MeshField_fromFile(vphifile, nx, ny, nz, SPEED);
  MeshField *gasvr = init_MeshField_fromFile(vrfile, nx, ny, nz, SPEED);
  MeshField *gasvtheta = init_MeshField_fromFile(vthetafile, nx, ny, nz, SPEED);
  ...
  // CoM is at origin;
  model->sunpos[0] = (-model->planetpos[0] * model->planetmass / MASS);
  model->sunpos[1] = (-model->planetpos[1] * model->planetmass / MASS);
  model->sunpos[2] = (-model->planetpos[2] * model->planetmass / MASS);

  // semi major axis of planet
  double sma = sqrt(model->planetpos[0] * model->planetpos[0] +
                    model->planetpos[1] * model->planetpos[1]);
  double hillRadius = sma * pow(model->planetmass / 3 / MASS, 1.0 / 3.0);
  double soundspeed = get_soundspeed(model, sma, MASS, R0);
  double bondiRadius = 2 * G * model->planetmass / soundspeed / soundspeed;
  // planet enevlope is min(hillRadius/4, bondiRadius)
  model->planetEnvelope =
      (hillRadius / 4. < bondiRadius) ? hillRadius / 4 : bondiRadius;
...
Model *init_Jupiter_Model(char *fargodir, char *nout, int rank) {
  ...
  model->omegaframe = 1 / sqrt(G * MSUN / R0 / R0 / R0);

  // get the planet(s) position(s)
  model->planetpos[0] = 5.2 * AU;
  ...
  double soundspeed = get_soundspeed(model, sma, MSUN, 5.2 * AU);
  double bondiRadius = 2 * G * model->planetmass / soundspeed / soundspeed;
  // planet enevlope is min(hillRadius/4, bondiRadius)
  model->planetEnvelope =
      (hillRadius / 4. < bondiRadius) ? hillRadius / 4 : bondiRadius;
  ...
  for (size_t k = 0; k < nz; k++) {
    ...
    for (size_t j = 0; j < ny; j++) {
      ...
      for (size_t i = 0; i < nx; i++) {
        ...
        vx = vr * cos(phi) * sin(theta) + -vphi * sin(phi) * sin(theta) +
             vtheta * cos(phi) * cos(theta);
        vy = vr * sin(phi) * sin(theta) + vphi * cos(phi) * sin(theta) +
             vtheta * sin(phi) * cos(theta);
...
// analytic values
double get_Omega(double r, double mstar) { return sqrt(G * mstar / r / r / r); }

double get_scaleheight(Model *model, double r, double R0) {
...
double get_soundspeed(Model *model, double r, double mstar, double R0) {
  double H = get_scaleheight(model, r, R0);
  double OM = get_Omega(r, mstar);
  return H * OM;
}
  ...
  for (size_t k = 0; k < model->nz; k++) {
    ...
    for (size_t j = 0; j < model->ny; j++) {
      ...
      for (size_t i = 0; i < model->nx; i++) {
        ...
        if (k == 0) {
          drhodtheta = (get_data(rho, k + 1, j, i) - get_data(rho, k, j, i)) /
                       (THETA[k + 1] - THETA[k]);
        } else if (k == model->nz - 1) {
          drhodtheta = (get_data(rho, k, j, i) - get_data(rho, k - 1, j, i)) /
                       (THETA[k] - THETA[k - 1]);
        } else {
          drhodtheta =
              (get_data(rho, k + 1, j, i) - get_data(rho, k - 1, j, i)) /
              (THETA[k + 1] - THETA[k - 1]);
        }

        if (j == 0) {
          drhodr = (get_data(rho, k, j + 1, i) - get_data(rho, k, j, i)) /
                   (R[j + 1] - R[j]);
        } else if (j == model->ny - 1) {
          drhodr = (get_data(rho, k, j, i) - get_data(rho, k, j - 1, i)) /
                   (R[j] - R[j - 1]);
        } else {
          drhodr = (get_data(rho, k, j + 1, i) - get_data(rho, k, j - 1, i)) /
                   (R[j + 1] - R[j - 1]);
        }

        if (i == 0) {
          drhodphi = (get_data(rho, k, j, i + 1) - get_data(rho, k, j, i)) /
                     (PHI[i + 1] - PHI[i]);
        } else if (i == model->nx - 1) {
          drhodphi = (get_data(rho, k, j, i) - get_data(rho, k, j, i - 1)) /
                     (PHI[i] - PHI[i - 1]);
        } else {
          drhodphi = (get_data(rho, k, j, i + 1) - get_data(rho, k, j, i - 1)) /
                     (PHI[i + 1] - PHI[i - 1]);
        }
        ...
        model->drhodx->data[idx] =
            drhodphi * dphidx + drhodr * drdx + drhodtheta * dthetadx;
        model->drhody->data[idx] =
            drhodphi * dphidy + drhodr * drdy + drhodtheta * dthetady;
        model->drhodz->data[idx] =
            drhodphi * dphidz + drhodr * drdz + drhodtheta * dthetadz;
